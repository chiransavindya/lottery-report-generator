...
<head>
    <meta charset="UTF-8">
    <title>{{ $lottery->name_en }} - Draw {{ $draw->draw_number }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    @php
        ...
        // Handle language-specific logos for some lotteries
        $langSpecificLogos = ['ds', 'lw', 'sr'];
        if (in_array($lotteryCode, $langSpecificLogos)) {
            $logoFile = "{$lotteryCode}_en.png";
        }
    @endphp

    <div class="ticket-container">
        <div class="ticket-header">
            <img src="{{ public_path("images/{$logoFile}") }}" alt="{{ $lottery->name_en }}" class="ticket-logo">

            <div class="draw-info-overlay">
                <span class="draw-label">Draw Number</span>
                <span class="draw-value">{{ $draw->draw_number }}</span>

                @if(isset($draw->metadata['color']))
                    <span class="color-label">Colour</span>
                    <span class="color-value">{{ $draw->metadata['color'] }}</span>
                @endif

                <div class="winning-section">
                    <div class="winning-title">
                        ----Winning Numbers----&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;English letter
                    </div>
                ...
                @if(isset($draw->metadata['next_jackpot']))
                    <div class="jackpot-bar">
                        Next Super Jackpot : Rs. {{ number_format($draw->metadata['next_jackpot'], 2) }}
                    </div>
                @elseif(isset($draw->jackpot_amount) && $draw->jackpot_amount > 0)
                    <div class="jackpot-bar">
                        Jackpot : Rs. {{ number_format($draw->jackpot_amount, 2) }}
                    </div>
                @endif
            </div>
        </div>
